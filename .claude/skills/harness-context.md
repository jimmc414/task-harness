...
Re-establish project knowledge for Task Harness when context is lost.
...
```bash
pytest --cov=harness --cov-report=term-missing  # Full suite
pytest tests/test_validators/ -v                 # Just validators
pytest tests/test_runner.py -v                   # Just runner
```
...
After reading this, confirm understanding and offer to help with:
1. Creating new pipelines
2. Creating new validators
3. Debugging failed runs
4. Running tests
5. Any other Task Harness development task
